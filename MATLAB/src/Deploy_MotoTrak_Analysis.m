...
[collated_file, ~, depfuns] = Vulintus_Collate_Functions(start_script,...
    collated_filename,...
    'DepFunFolder','on',...
    'RemoveOrphans','on');                                                  %Call the generalized function-collating script.

for i = 1:length(depfuns)                                                   %Step through each function dependency.
    [~, shortfile, ~] = fileparts(depfuns{i});                              %Grab the function filename minus the path and extension.
    if any(strcmpi(shortfile,{'MotoTrakFileRead','ArdyMotorFileRead'}))     %If the function is a file-reading function...
        copyfile(depfuns{i},path,'f');                                      %Create a copy of the function in the main MATLAB folder.
    end
end
